...
const uploadFile = (parser) => {
  parser.stream.pipe(fs.createWriteStream(parser.filePath).on('error', cwsErr => {
    if (cwsErr && !(cwsErr.code === 'ENOENT')) throw new _U_.error.ServerError(cwsErr);
    fs.mkdir(path.dirname(parser.filePath), mkdirErr => {
      if (mkdirErr) throw new _U_.error.ServerError(92001, `${path.dirname(parser.filePath)}\n${mkdirErr.toString()}`);
...
const transpileRaw = (parser, configs) => {
  if (checkStatus(parser, configs)) {
    return _U_.object.assignDeep(
      {}, parser.fieldName,
...
const fieldParser = (req, res, configs, args) => {
  ...
  if (value) {
    _U_.object.assignDeep(req.body.busboySlip.raw, fieldName, _U_.string.toEscapedChars(value), { mutate: true });
  }
};
