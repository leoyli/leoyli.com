...
router
    ...
    .post(_pre.wrapAsync(async (req, res) => {
...
router
    ...
    .get(_pre.wrapAsync(async (req, res) => {
    ...
    .patch(_pre.wrapAsync(async (req, res) => {
        req.body.post.content = _fn.string.escapeInHTML(req.body.post.content)
        await PostModel.findByIdAndUpdate(_fn.string.readObjectID(req.url), req.body.post, {new: true});
        req.flash('info', 'Post have been successfully updated!');
        res.redirect(`/post/${req.body.post._id}`);
    }))
    .delete(_pre.wrapAsync(async (req, res) => { // todo: trash can || double check
...
router
    .get('/editor/:KEY', _pre.isAuthorized, _pre.wrapAsync(async (req, res) => {
...
// show
router
    .get(/^\/[a-f\d]{24}(\/)?$/, _pre.wrapAsync(async (req, res) => {
    ...
    .get('/:KEY', _pre.wrapAsync(async (req, res) => {
